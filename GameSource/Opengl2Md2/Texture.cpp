...
Texture::Texture ()
	: _handle (0), _Alphahandle(0), _flags (kDefault),
...
Texture2D::Texture2D (const string &filename, const string &alphaTexture, TextureFlags flags)
{
	try
	{
		// 알파 블랜딩을 하기위 하여 이미지 불러옴
		ImageBuffer ibuff_a (alphaTexture);
		// 이미지 펙토리에서 이미지를 가져오기위해 텍스트 버퍼
		ImageBuffer ibuff_c (filename);

		auto_ptr<Image> img (ImageFactory::createImage (ibuff_c,ibuff_a));

		// Create texture from image buffer
		create (img.get (), flags);
		_AlphaTextureRealPath = alphaTexture;
	}
	catch (std::exception &err)
	{
		cerr << "Error: Couldn't create texture 2D from "
	...
	Texture2D::create (const Image *img, TextureFlags flags)
{
	...
	//glTexParameteri (GL_TEXTURE_2D, GL_TEXTURE_ALPHA_TYPE,GL_ALPHA);
...
void
	Texture2D::create (const Image *img, const Image *ImageAlpha, TextureFlags flags)
{
	if (!img->pixels ())
		throw std::runtime_error ("Invalid image data");

	// Fill texture's vars
	_name = img->name () + ImageAlpha->name();
	_TextureRealPath = img->name();
	_AlphaTextureRealPath = ImageAlpha->name();
	_flags = flags;
	_standardCoordSystem = img->stdCoordSystem ();
	...
	{
		// No hardware mipmap generation support, fall back to the
		// good old gluBuild2DMipmaps 
		//glBlendFunc(GL_ONE_MINUS_SRC_COLOR, GL_ZERO);
		
		//glBlendFunc(GL_ONE , GL_ZERO);

		//glBindTexture (Alphahandle(), _Alphahandle);
		
	
		gluBuild2DMipmaps (GL_TEXTURE_2D, GL_RGBA,
			ImageAlpha->width (), ImageAlpha->height (), GL_RGBA,
			GL_UNSIGNED_BYTE, ImageAlpha->pixels());
