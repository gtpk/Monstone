...
ImageFreeImage::ImageFreeImage (const ImageBuffer &ibuff) : dib(0)
{
	//image format
	...
	fif = FreeImage_GetFileType(_name.c_str(), 0);
	//if still unknown, try to guess the file format from the file extension
	if(fif == FIF_UNKNOWN) 
		fif = FreeImage_GetFIFFromFilename(_name.c_str());
	//if still unkown, return failure
	if(fif == FIF_UNKNOWN)
		throw ImageException ("FreeImage Said Unknown  FIF_UNKNOWN", _name);

	//check that the plugin has reading capabilities and load the file
	if(FreeImage_FIFSupportsReading(fif))
		dib = FreeImage_Load(fif, _name.c_str());
	//if the image failed to load, return failure
	if(!dib)
		throw ImageException ("FreeImage Said I Can't Loade ", _name);

	//retrieve the image data
	_pixels = FreeImage_GetBits(dib);
	//get the image width and height
	_width = FreeImage_GetWidth(dib);
	_height = FreeImage_GetHeight(dib);
	//if this somehow one of these failed (they shouldn't), return failure
	if((_pixels == 0) || (_width == 0) || (_height == 0))
		throw ImageException ("FreeImage Said Empty File ", _name);
	/*
...
ImageColortoAlpha::ImageColortoAlpha (const ImageBuffer &ibuff,const ImageBuffer &aibuff)
{

	Image *colorresult = new ImagePCX (ibuff);
	...
	//크기 우선권은 컬러 맵이 가진다.
	_width = colorresult->width();
	_height = colorresult->height();


	//난 알파 넣을꺼임
	const GLubyte* buffer = alpharesult->pixels();
	const GLubyte* mPixels = colorresult->pixels();

	_pixels = new GLubyte[Imagesize];
	int i=0;
	int j=0;
	for(; i<Imagesize; i+=4, j+=3)
	{
		_pixels[i] = mPixels[j];
		_pixels[i+1] = mPixels[j+1];
		_pixels[i+2] = mPixels[j+2];
 
		//평균값으로 한다 보나마나 같겠지만.
		//GLubyte average = (buffer[i+2] + buffer[i+1] +buffer[i])/3;
